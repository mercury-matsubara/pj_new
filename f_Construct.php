...
function getAutoUpdateValue( $setting_value, $post )
{
    ...
    if ($pos == false) {
        ...
        $outvalue = '';
        if ($scope === 'session') {
            if (array_key_exists('SYAMEI', $_SESSION) === false) { //未ロードの場合は自社情報読み出し
                loadJisyaMaster();
            }
...
function kousuSQL($syain,$day){
    ...
    $sql = "SELECT PROJECTNUM,EDABAN,PJNAME,KOUTEIID,KOUTEINAME,TEIZITIME,ZANGYOUTIME,7PJSTAT 
            ...
            WHERE convert(replace(replace(syaininfo.STAFFID,' ',''),'　','') using utf8) COLLATE utf8_unicode_ci LIKE '$syain' 
